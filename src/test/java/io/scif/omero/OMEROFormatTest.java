...
public class OMEROFormatTest {
    ...
    /**
     * Tests {@link OMEROFormat#parseArguments} and create a session
     * passing an existing session ID and an existing client.
     */
    @Test
    ...
    /**
     * Tests {@link OMEROFormat#parseArguments} and create a session
     * passing an existing session ID and an existing client. The client
     * specified does not correspond to the session ID.
     * */
    @Test
    public void testOMEROSessionFromSessionIDSpecifyClients() throws FormatException {
        final OMEROFormat omeroFormat = getFormat();
        final MetadataService metadataService =
            omeroFormat.context().service(MetadataService.class);
        final String name = null;
        ...
        try {
            ...
            assertTrue(client.getProperty("omero.host").trim().length() > 0);
            OMEROSession session = new OMEROSession(meta.getCredentials(), c);
            String sessionID = session.getClient().getSessionId();
            assertTrue(sessionID.length() > 0);
            session.close();
            client.closeSession();
     ...
     * Tests {@link OMEROFormat#parseArguments} and create a session
     * passing an existing session ID and an existing client. The client
     * specified does not have an active session.
     */
    @Test
    public void testOMEROSessionFromSessionIDSpecifyClientsWithoutActiveSession() throws FormatException {
        final OMEROFormat omeroFormat = getFormat();
        final MetadataService metadataService =
            omeroFormat.context().service(MetadataService.class);
        final String name = null;
